...
import 'package:alwali_driver/src/network/dio_exception.dart';

class DioClient {
  static final _baseOptions = BaseOptions(
    baseUrl: 'http://213.136.71.7/alnabali/public/api/android',
  ...
  static Future<dynamic> postDailyTripToday(String id) async {
    ...
    var dio = Dio(_baseOptions);
    dio.options.headers['X-CSRF-TOKEN'] = token;
    print("new server connected"+id+currentLang.toString());
    // ignore: avoid_print
    try {
      final response = await dio.post(
        '/daily-trip/today',
  ...
  static Future<dynamic> postDailyTripLast(String id) async {
    ...
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    final currentLang = sharedPreferences.getString("lang");
    print("Past server connected"+id+currentLang.toString());
    var dio = Dio(_baseOptions);
    dio.options.headers['X-CSRF-TOKEN'] = token;
